...
configuration for either Elasticsearch (ELK stack) or Google Stackdriver.
[Learn more about the Fluentd container image and requirements](./fluentd/README.md).

If you already have an observability plugin installed, see the
following topics for details configuring your plugin or accessing your data:

  * [Learn about configuring your observability plugin](setting-up-a-logging-plugin.md).
  * [Learn how to access your data](#whats-next).

## Table of contents

* [Before you begin](#before-you-begin)
* [Viewing which observability plugin is installed](#viewing-which-observability-plugin-is-installed)
* [Choosing and installing an observability plugin](#choosing-and-installing-an-observability-plugin)
...
* If no pods are listed and you receive the `No resources found`, see the section below to
  about [choosing an observability plugin](#choosing-and-installing-a-monitoring-plugin).

* If an observability plugin is installed, the following common pods are
  listed:
    ...
    prometheus-system-1
    ```
    The Stackdriver plugin uses only the common set of pods. See the
    [Monitoring with Stackdriver]](#monitoring-with-stackdriver-prometheus-and-grafana)
    section for more information.

    If the following pods are also listed, see the
    [Monitoring with Elasticsearch](#monitoring-with-elasticsearch-kibana-prometheus-and-grafana)
    section for more information about the ELK stack:
...
You can choose from either an [ELK stack](https://www.elastic.co/elk-stack), or
[Google Stackdriver](https://cloud.google.com/stackdriver/) observability plugin
to monitor your Knative cluster. Both plugins utilize Prometheus and Grafana for
visualizing metrics data.

**Important**: Simultaneously monitoring your Knative cluster with both
plugins is unsupported. You can install and configure only a single observability
plugin per Knative Serving installation.
...
The observability plugins require a [Fluentd](./fluentd/README.md) based container
image. You can either


1. Install a default observability plugin package:
1. Install an individual observability feature package:
1. Install a custom observability plugin:
...
* Monitoring with Elasticsearch, Kibana, Prometheus, and Grafana
  ...
  **Note**: This daemonset configuration gets installed by default if you run the
...
### Installing a default observability plugin

Default: [monitoring.yaml](https://github.com/knative/serving/releases/download/v0.2.2/monitoring.yaml)
  ...
  ```shell
  elasticsearch-logging-0
  elasticsearch-logging-1fluentd-ds-5kc85
  fluentd-ds-vhrcq
  fluentd-ds-xghk9
  grafana-798cf569ff-v4q74
  kibana-logging-7d474fbb45-6qb8x
  ...
  zipkin-7bc4d7c99-tf4dj   1/1    Running  0         1h
  ```

[monitoring-logs-elasticsearch.yaml](https://github.com/knative/serving/releases/download/v0.2.2/monitoring-logs-elasticsearch.yaml)
  ...
  elasticsearch-logging-0
  elasticsearch-logging-1
  kibana-logging-7f7b9698bc-xd7n2
  ```

[monitoring-metrics-prometheus.yaml](https://github.com/knative/serving/releases/download/v0.2.2/monitoring-metrics-prometheus.yaml)
...
[monitoring-tracing-zipkin-in-mem.yaml](https://github.com/knative/serving/releases/download/v0.2.2/monitoring-tracing-zipkin-in-mem.yaml)
[monitoring-tracing-zipkin.yaml](https://github.com/knative/serving/releases/download/v0.2.2/monitoring-tracing-zipkin.yaml)
...
### Optional: Creating a custom `fluentd` container image

Knative supported observability plugins are based on [Fluentd](./fluentd/README.md).
You can use the provided and pre-configured Fluentd image as is, modify the provided Fluentd container image, or create your own Fluentd based container image.
...
To install the observability plugin:
...
See [What's next](#whats-next) to learn how to access your data.
...
After you install and configure an observability plugin, use the following
documents to learn how to access your data:
